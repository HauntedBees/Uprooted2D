...
function GetEnemy(name) {
    switch(name) {
		...
		/* South City */
		case "mrbruno": return new EnemyDetail(GetDisplayName(name, 1), "md", 27, { dx: 0.05, dy: -0.15, w: 0.25, h: 0.9 }, 10, 1, 1, 4, 3, true, [0, 0, 1, 0], "mobster", "tomato,grapes", [ { money: true, min: 600, max: 600 }, { seed: "tomato", min: 10, max: 10 }, { seed: "grapes", min: 10, max: 10 } ], { tile: "dirt" });
		case "mobsty1": return new EnemyDetail(GetDisplayName(name, 3), "md", 27, { dx: 0.05, dy: -0.15, w: 0.25, h: 0.9 }, 10, 1, 1, 4, 1, false, [0, 1, 1, 1], "mobster", "tomato,grapes", [ { money: true, min: 300, max: 700 }, { seed: "tomato", min: 5, max: 15 }, { seed: "grapes", min: 5, max: 15 } ], { tile: "dirt" });
		case "mobsty2": return new EnemyDetail(GetDisplayName(name, 3), "md", 28, { dx: 0.05, dy: -0.15, w: 0.25, h: 0.9 }, 10, 1, 1, 4, 1, false, [0, 1, 1, 1], "mobster", "tomato,grapes", [ { money: true, min: 600, max: 1200 }, { seed: "tomato", min: 10, max: 20 }, { seed: "grapes", min: 10, max: 20 }, { seed: "portobello", min: 5, max: 10 } ], { tile: "dirt" });
		case "mobBoss": return new EnemyDetail(GetDisplayName(name, 3), "md", 28, { dx: 0.05, dy: -0.15, w: 0.25, h: 0.9 }, 10, 1, 1, 5, 3, true, [0, 1, 0, 0], "basic", "tomato,grapes,arborio,portobello", [ { money: true, min: 2000, max: 2000 }, { seed: "tomato", min: 30, max: 30 }, { seed: "grapes", min: 30, max: 30 }, { seed: "arborio", min: 30, max: 30 } ], { tile: "dirt" });
		/* Bee Queen */
...
debug.AllEnemies = ["Discussly", "robo", "bigBot", "robo2", "robo3", "ScienceMan", "mouse", "sqorl", "bear", "turky", "bossturky", "Worker", "BossWorker", "kelpBoy", "fishFace", "seaMonk", "seaHandR", "seaMan", "seaHandL", "chickBot", "piggun", "golem", "lawnmower", "machineA", "machineB", "machineC", "machineD", "router", "server", "housekeeper", "outlet", "mrbruno", "mobsty1", "mobsty2", "mobBoss", "beeQueenA", "beeQueenB", "beeQueenC"];